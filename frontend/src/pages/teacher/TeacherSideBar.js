...
const TeacherSideBar = () => {
  ...
  const teacherSidebarItems = [
    ...
    { label: sclassName ? `الصف: ${sclassName.sclassName}` : 'الصف', icon: <ClassOutlinedIcon />, link: '/Teacher/class' },
    { label: 'الشكاوى', icon: <AnnouncementOutlinedIcon />, link: '/Teacher/complain' },
    { label: 'الجدول الدراسي', icon: <ClassOutlinedIcon />, link: '/Teacher/schedule' },
    { label: 'بيانات المعلم', icon: <AccountCircleOutlinedIcon />, link: '/Teacher/profile' },
  ];
  ...
  return (
    <Box sx={{
      ...
      {/* Nav Items */}
      <Box sx={{ flex: 1 }}>
        {teacherSidebarItems.map((item, idx) => {
